...
$(function(){
	$("#FusionCharts").wBox({
		...
		target:"stockincomeBillItem_chart1.action?baseQuery.beginDate="+$("#beginDate").val()
	...
	$("#HighCharts").wBox({
		...
		target:"stockincomeBillItem_chart3.action?baseQuery.beginDate="+$("#beginDate").val()
				+"&baseQuery.endDate="+$("#endDate").val()
				+"&baseQuery.status="+$("#status").val()
				+"&baseQuery.groupBy="+$("#groupBy").val()
	});
...
<body>
<s:form id="domainForm" action="stockincomeBillItem_query">
...
<!--     <td>状态:</td> -->
    <td>
    	...
    	<s:select id="groupBy" list="#{'o.bill.supplier.name':'供应商分组','o.bill.keeper.name':'采购员分组','month(o.bill.vdate)':'月份'}" name="baseQuery.groupBy"/>
